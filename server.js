...
// during testing (allow all)
app.use(cors());
// or lock to Netlify when you’re ready:
// app.use(cors({ origin: ['http://localhost:8888', 'https://YOUR-NETLIFY-SITE.netlify.app'] }));
...
const ALLOWED_ORIGINS = [
  'https://halalfreshnfast.netlify.app', // ✅ replace with your real Netlify URL
  'http://localhost:3000',
  'http://localhost:4000',
];
  ...
  try {
    fs.watch(pricesPath, { persistent: false }, () => {
      setTimeout(() => {
        try {
          loadPriceBookFromFile();
          console.log('🔁 Reloaded data/prices.json');
        } catch (e) {
          console.warn('⚠️ Failed to reload prices.json:', e.message);
        }
      }, 200);
    });
  } catch (_) {}
...
app.use(express.static(__dirname)); // serve index.html, menu.html, order.html
...
const sanitizeCart = (cart) =>
  cart.map(({ name, qty }) => {
    ...
    if (!Number.isInteger(qty) || qty <= 0)
      throw new Error(`Bad qty for ${name}`);
...
app.post('/api/checkout', async (req, res) => {
  try {
      ...
      if (typeof body.address === 'string') {
        ...
        delivery_address = {
          ...
          administrative_district_level_1:
            body.address.administrative_district_level_1,
    ...
    // Totals
    const subtotalCents =
      sumCents(items) + (isDelivery ? DELIVERY_FEE_CENTS : 0);
    const taxCents =
      TAX_RATE_PERCENT > 0
        ? Math.round(subtotalCents * (TAX_RATE_PERCENT / 100))
        : 0;
    ...
    res
      .status(400)
      .json({ success: false, error: e.message || 'Checkout failed' });
  }
});
