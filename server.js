...
// Use built-in fetch on Node ≥18, otherwise lazy-load node-fetch
const fetch =
  global.fetch ||
...
/* -------------------------------------------------------
   CORS (allow all while finishing setup; lock later)
-------------------------------------------------------- */
app.use(cors());          // open during setup
app.options('*', cors()); // handle preflight (important for Netlify → Render)

/* -------------------------------------------------------
   Middleware & static files
-------------------------------------------------------- */
app.use(express.json());
app.use(express.static(__dirname)); // serves index.html, menu.html, order.html, images, css, etc.
...
/* -------------------------------------------------------
   ...
   You can also place data/prices.json to override these.
-------------------------------------------------------- */
...
// Allow external override via data/prices.json (hot-reload)
...
const CheckoutBody = z.object({
  ...
  pickupTime: z.string().optional(), // ISO string
